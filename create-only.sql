...
INSERT INTO tax_categories (name, keywords, schedule_c_line)
SELECT name, keywords, schedule_c_line
FROM (VALUES
...
) AS v(name, keywords, schedule_c_line)
WHERE NOT EXISTS (
  SELECT 1 FROM tax_categories WHERE name = v.name
);
...
INSERT INTO subscription_plans (name, price, billing_period, features, max_transactions, max_connections, nexus_monitoring_level, is_featured)
SELECT name, price, billing_period, features, max_transactions, max_connections, nexus_monitoring_level, is_featured
FROM (VALUES
  ('Starter', 29.00, 'mo', '["Up to 500 transactions/mo", "Basic Nexus monitoring", "1 Shopify store connection", "Email support"]'::jsonb, 500, 1, 'basic', FALSE),
  ('Growth', 79.00, 'mo', '["Unlimited transactions", "Advanced Nexus Sentinel", "Amazon, Shopify & Stripe", "IRS-ready Schedule C", "Priority CPA support"]'::jsonb, NULL, 3, 'advanced', TRUE)
) AS v(name, price, billing_period, features, max_transactions, max_connections, nexus_monitoring_level, is_featured)
WHERE NOT EXISTS (
  SELECT 1 FROM subscription_plans WHERE name = v.name
);
...
-- Nexus Status RLS
DROP POLICY IF EXISTS "Users can view own nexus status" ON nexus_status;
...
DROP POLICY IF EXISTS "Anyone can view tax categories" ON tax_categories;
